...
function _writeHeaders(
  headers: AuthHeaders | Promise<AuthHeaders>
...
globalThis.addEventListener("message", (ev: MessageEvent): boolean => {
  ...
  console.log(
    `NEUROGLANCER: ${
      typeof Window == "function" ? "window" : "worker"
    } just got a token as a message!`
  );
...
async function fetchtFromDataProxy({
  url,
  init,
  ...
  if (
    ...
    (url.path.raw.startsWith("/api/buckets/") || url.path.raw.startsWith("/api/v1/buckets/"))
  ) {
    fixedUrl = url.updatedWith({ extra_search: new Map([["redirect", "false"]]) });
  }

  const authorization_header_key = "Authorization";
  let fixedHeaders = mergeHeaders(init?.headers, {
